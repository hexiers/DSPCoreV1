...
void InitECap(void)
{
	ECap4Regs.TSCTR = 0x0;//初始化时基
	ECap4Regs.CTRPHS = 0;
	ECap4Regs.ECCTL1.bit.FREE_SOFT = 2;
	ECap4Regs.ECCTL1.bit.PRESCALE = 0;//不分频
	ECap4Regs.ECCTL1.bit.CAPLDEN = 0;//禁止捕获===================>后续需开启
	ECap4Regs.ECCTL1.bit.CTRRST1 = 1;//捕获后清除计数器
	ECap4Regs.ECCTL1.bit.CTRRST2 = 1;//捕获后清除计数器
	ECap4Regs.ECCTL1.bit.CTRRST3 = 1;//捕获后清除计数器
	ECap4Regs.ECCTL1.bit.CTRRST4 = 1;//捕获后清除计数器
	ECap4Regs.ECCTL1.bit.CAP1POL=0;//上升沿触发
	ECap4Regs.ECCTL1.bit.CAP2POL=1;//下降沿触发
	ECap4Regs.ECCTL1.bit.CAP3POL=0;//上升沿触发
	ECap4Regs.ECCTL1.bit.CAP4POL=1;//下降沿触发
	
	ECap4Regs.ECCTL2.bit.CAP_APWM = 0;//捕获模式,非APWM模式
	ECap4Regs.ECCTL2.bit.STOP_WRAP = 3;//循环捕获
	ECap4Regs.ECCTL2.bit.CONT_ONESHT= 0;//连续捕获模式
	#if EN_PWMINPUT
	//用于PWM指令输入
	ECap1Regs.TSCTR = 0x0;//初始化时基
	ECap1Regs.CTRPHS = 0;
	ECap1Regs.ECCTL1.bit.FREE_SOFT = 2;
	ECap1Regs.ECCTL1.bit.PRESCALE = 0;//不分频
	ECap1Regs.ECCTL1.bit.CAPLDEN = 0;//禁止捕获===================>后续需开启
	ECap1Regs.ECCTL1.bit.CTRRST1 = 1;//捕获后清除计数器
	ECap1Regs.ECCTL1.bit.CTRRST2 = 0;//捕获后不清除计数器
	ECap1Regs.ECCTL1.bit.CTRRST3 = 1;//捕获后清除计数器
	ECap1Regs.ECCTL1.bit.CTRRST4 = 0;//捕获后不清除计数器
	ECap1Regs.ECCTL1.bit.CAP1POL = 1;//下降沿触发
	ECap1Regs.ECCTL1.bit.CAP2POL = 0;//上升沿触发
	ECap1Regs.ECCTL1.bit.CAP3POL = 0;//上升沿触发
	ECap1Regs.ECCTL1.bit.CAP4POL = 1;//下降沿触发
	
	ECap1Regs.ECCTL2.bit.CAP_APWM = 0;//捕获模式,非APWM模式
	ECap1Regs.ECCTL2.bit.STOP_WRAP = 1;//循环捕获
	...
	ECap1Regs.ECCTL1.bit.CAPLDEN = 0;//禁止捕获===================>后续需开启
	ECap1Regs.ECCTL1.bit.CTRRST1 = 1;//捕获后清除计数器
	ECap1Regs.ECCTL1.bit.CTRRST2 = 1;//捕获后清除计数器
	ECap1Regs.ECCTL1.bit.CTRRST3 = 1;//捕获后清除计数器
	ECap1Regs.ECCTL1.bit.CTRRST4 = 1;//捕获后清除计数器
	ECap1Regs.ECCTL1.bit.CAP1POL=0;//上升沿触发
	ECap1Regs.ECCTL1.bit.CAP2POL=0;//上升沿触发
	ECap1Regs.ECCTL1.bit.CAP3POL=0;//上升沿触发
	ECap1Regs.ECCTL1.bit.CAP4POL=0;//上升沿触发
	
	ECap1Regs.ECCTL2.bit.CAP_APWM = 0;//捕获模式,非APWM模式
	ECap1Regs.ECCTL2.bit.STOP_WRAP = 3;//循环捕获
	ECap1Regs.ECCTL2.bit.CONT_ONESHT= 0;//连续捕获模式
	#endif
}
